    ...
    def _setup_axes(self, subfig):
        ...
        self.token_node_map = {
            token: list(
                sorted([
                    (
                        int(key), node["model_name"]
                    )  # lets hope that the nodeIds are sorted... as this is very hacky atm
                    for key, node in self.graph['nodes'].items()
                    if node["model_name"].startswith(f'{token}_')
                ])
            )  # lets hope, that the <token>_<atom>-<stateId> convention holds...
        ...
        token_sub_colors = {
            token: [
                self.atom_colors[self._hack_get_atom_from_model_id(atom[1])]
                for atom in self.token_node_map[token]
            ]
            for token in self.tokens
        }
        ...
        for ax, token in zip(self.axes, self.tokens):
            ...
            self.bar_objs.append(
                ax.broken_barh(verts,
                               yrange=(0, 1),
                               facecolors=token_sub_colors[token]))
    ...
    def _init_draw(self, subfig):
        ...
        def update(hypothesis=None, hmm_meta=None):
            ...
            if hmm_meta is not None and not self._axes_setup:
                ...
                self.token_colors, self.atom_colors, self.state_colors = self._init_colors(
                    self.topology)
            ...
            if self._axes_setup and hypothesis is not None:
                ...
                res_changed_bars = [
                ]  # faster if only few bars are changed, albeit less elegant than before
                for i, bar in enumerate(self.bar_objs):
                    alphas = [
                        1 if x[0] in state_ids else alpha_val
                        for x in self.token_node_map[self.tokens[i]]
                    ]
                    all_alphas = alphas + self.previous_alphas[i]
                    self.previous_alphas[i] = alphas
                    if sum(all_alphas) > len(
                            all_alphas
                    ) * alpha_val:  # if one alpha is higher than alpha_val this will return true
