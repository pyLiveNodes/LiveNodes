    ...
    def process(self, data, _ctr, **kwargs):
        ...
        if self.concat_batches:
            ...
            if to_window.shape[0] >= self.length:
                res = self.__sliding_window(to_window, size=self.length, stepsize=self.stepsize, axis=0, copy=True)
                res = res.transpose((0, 2, 1))
                # self.debug(res[0,:,0])
                used_samples = self.length + (res.shape[0] - 1) * (self.length - self.overlap)
                ...
                self.ctrs = {key: val for key, val in self.ctrs.items() if val >= used_samples}
