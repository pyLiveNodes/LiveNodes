...
class Transform_window(Node):
    ...
    def __init__(self, length, overlap, concat_batches=True, name = "Window", **kwargs):
    ...
    def __sliding_window(self, data, size, stepsize = 1, axis = -1, copy = True):
        ...
        if axis >= data.ndim:
            raise ValueError(
                f"Axis {axis} out of range for dim {data.ndim}"
            )
    ...
    def process(self, data):
        ...
        if to_window.shape[0] >= self.length:
            res = self.__sliding_window(to_window, size=self.length, stepsize=self.stepsize, axis=-2, copy=True)
            # self.debug(res[0,:,0])
            used_samples = self.length + len(res) * (self.length - self.stepsize)
            ...
            self._emit_data(res.transpose((0, 2, 1)))
            return False # prevent_tick? -> no we did emit data here
        return True
