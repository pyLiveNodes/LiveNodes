...
class FeatureUnion (BaseTransformer_lazy):
    def __init__(self, featureInstances: tp.List[BaseEstimator], featureNames=[], safe=True, batch_size=50, **kwargs):
    ...
    def transformTS(self, ts):
        return pipeline.make_union(*self.featureInstances, **self.unionParams).transform(ts)
    ...
    def transform_mapper(self, session):
        raise Exception('Transform Mapper not implemented to standard') # <- remove for perf estimation, but don't use in production!
        # problems: 
...
#         2. code assume features are independent of windows (should hold anyway, but this actually leads to false results if violated)
        ...
        for batch in batch_generator(session, self.batch_size):
            ...
            tmp = np.concatenate([ft.transform(np.array(slc)) for ft in self.featureInstances], axis=1)        
            res = []
            idx = 0
            for x in batch:
    ...
    def transform(self, session: tp.Iterable) -> tp.List:
        ...
        for i, ft in enumerate(self.featureInstances):
            ft.transform(toTransform) # required to access the number of dimensions of this feature
        ...
        out_cols = [f"{column}__{feature}" for feature in features for column in columns]        
        if self.safe:
