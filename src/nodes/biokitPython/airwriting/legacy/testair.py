...
class AirwritingLoader:
    ...
    def __init__(self, dictionary, grammarfile):
        ...
        self.modelMapper = BioKIT.ModelMapper.ReadTopology(
            self.cacheScorer, "distrib_tree", 
            "topology_tree", "topologies",
            "transitions")
        ...
        self.beams = BioKIT.Beams(self.hypoBeam,
                                   self.hypoTopN,
                                   self.activeNodeBeam,
                                   self.activeNodeTopN,
                                   self.finalNodeBeam,
    ...
    def preprocessing(self, adcPath):
        ...
        data = self.inputDataFileReader.readFile(adcPath,
             BioKIT.SF_FORMAT.RAW | BioKIT.SF_FORMAT.PCM_16, samplingRate, numChannels, 0, 0, adcOffset)
    ...
    def forcedAlignment(self, mcfs, reference):
        ...
        self.utterancedecoder = BioKIT.Decoder(self.utteranceSearchGraphHandler)
        ...
        modelNames = [self.cacheScorer.getModelName(self.utteranceSearchGraphHandler.getSearchGraph().getNode(nid).getModelId()) for nid in uniqNodeIds]
        mappedNodeIds = [uniqNodeIds.index(i) for i in nodeIds]
        for pi in forcedpath:
            print("modelId: %s , partialScore: %s" % (pi.mModelId, pi.mPartialScore))
        plotpath = visualization.PlotPath(forcedpath,
                                          multiChannel[0],
                                          self.utteranceSearchGraphHandler.getSearchGraph(),
                                          self.gmmScorer)
    ...
    def search(self, adcPath, reference):
        ...
        plotpath = visualization.PlotPath(path,
                                          multiChannel[0],
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description="Load Airwriting system from" + 
                                     " description files")
