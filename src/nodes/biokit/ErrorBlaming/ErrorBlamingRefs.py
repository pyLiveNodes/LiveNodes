...
class TestIntegrationTestSpeech(unittest.TestCase):
    ...
    def test_allTestData(self):
      ...
      pathPrefix = "/home"
      databasePath = os.path.join(pathPrefix, 'telaar/db.newDecoderRelativePaths/speech.db')
	  ...
	  while (i < len(infoList)):
		  ...
		  for token in tokens:
			  ...
			  if(tokenId == -1):
				  skipUtterance = True
				  log("Info","Skipped utterance, because of invalid token")
				  break
			  tokenIds.append(tokenId)

		  if(skipUtterance != True):
		    searchGraphHandler = SearchGraphHandler(dictionary,tokenIds,
		    dictionary.getBaseFormId("$"), modelMapper, cacheScorer,
		    beams, tokenSequenceModelWeight, tokenInsertionPenalty)
		    ...
		    snapshotName = refSnapshotPath + "/" + str(info['UTTID']) + ".snp"
		    searchGraphHandler.createSnapshot(snapshotName,tokenSequenceModelWeight,tokenInsertionPenalty)
		    log("Info", "created Snapshot file")

		    if(os.path.isfile(adcPath) != True):
			log("Warning", "File "+info['ADC'] + "does not exist!")
		    else:
			    ...
			    if(len(result) == 0):
			     ...
			     if(beamFactor > beamMax):
				     ...
				     log("Info", "Skipping utterance, deleting reference snapshot file")
		             else:
				     i = i - 1; #decrement i to redo the utterance
				     log("Info", "Increasing beam factor to " + str(beamFactor))
