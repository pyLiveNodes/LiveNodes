...
if __name__ == "__main__":
    ...
    parser.add_argument('--type', choices = ['character', 'sentence',
                        'trainchar', 'words'], default = 'character')
    ...
    with open(logfile, "w") as logfh:
        if args.type == 'character':
            ...
            retcode = subprocess.call(cmd, env=environ, 
                                      stderr=subprocess.STDOUT, stdout=logfh)
            ...
            params = [x + " = " + config._sqlize(config[x]) for x in identparams]
            whereexp = " AND ".join(params) + " AND nriter = " + str(iterations)
            sqlcmd = "SELECT distribWeights, codebookWeights FROM %s WHERE %s" % (args.modeltable, whereexp)
            dbres = [x for x in controlDb.executeStatement(sqlcmd)]
            print(dbres)
            assert(len(dbres) == 1)
            modeldir = os.path.dirname(dbres[0]['distribWeights'])
            modelFileNames = ["codebookSet", "dummycbw", "distribSet",
                              "dummydss", "topologies", "topologyTree",
                              "distribTree", "phonesSet", "transitionModels"]
            models = list(zip(airwritingConfig.JanusConfig.modelFileKeys,
    ...
    for iter in range(config['iterations']+1):
        ...
        config['distribWeights'] = os.path.abspath('distribWeights.'+str(iter))
        config['codebookWeights'] = os.path.abspath('codebookWeights.'+str(iter))
        config['nriter'] = iter
        results = None
