...
class WKM:
  ...
  """
    ...
    def getClusterSamples(self, index):
        ...
        l = self.boundaries[index +
                            1] if (index + 1 < self.numclusters) else len(
                                self.samples)
    ...
    def incrementalMeans(self, sample, j, b, n, m):
        ...
        for d in range(self.dimensions):
            newb[d] = self.centroids[b][d] + (sample[d] -
                                              self.centroids[b][d]) / (m + 1.0)
            newj[d] = self.centroids[j][d] - (sample[d] -
                                              self.centroids[j][d]) / (n - 1.0)
        ...
        while True:
            ...
            for j in range(self.numclusters):
                if j > 0:
                    ...
                    for i in range(
                            0,
                            int(math.floor(n / 2.0 * (1 - self.threshold))) +
                            1):
                        p = c[i]
                        b = j - 1
