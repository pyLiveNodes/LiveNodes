...
class AirwritingDb(Database.Database):
    ...
    createSqlStatements = {
        ...
        "hand":
        "CREATE TABLE hand (id INTEGER, name TEXT, PRIMARY KEY (id))",
        "experiments":
    ...
    def createViewCVResults(self,
        ...
        sqlResults = (
            "CREATE VIEW IF NOT EXISTS " + viewResults +
            " AS SELECT avg(wer) AS avgwer, sum(cputime) AS sumtime, * FROM " +
            resultTable + " GROUP BY " + groupby + " HAVING count(*) = " +
            str(foldNrs))
    ...
    def insertPerson(self, name, hand):
        ...
        if (result.size() == 0):
            sqlcmd = "INSERT INTO " + self.persons + " (name, dominantHand) VALUES " + "( \"" + name + "\", " + str(
                hand) + " )"
            result = self.executeStatement(sqlcmd)
            self.logres(result)
    ...
    def addExperimentFromDirectory(self,
                                   personName,
                                   experimentName,
                                   directory,
                                   fileSuffix,
    ...
    def convertToNewDb(self,
                       filename,
                       has_corrupt_rec=False,
        ...
        for experiment in experiments:
            person = airdb.session.query(
                adb.Person).filter_by(name=experiment['personname']).one()
            print(person)
            #bad hack next to lines
            basedir = '/'.join(experiment['baseDir'].split('/')[3:])
            string_id = experiment['stringId'][1:]
            newexp = adb.Experiment(string_id=string_id, base_dir=basedir)
            ...
            try:
                ...
                airdb.session.rollback()
        recordings = self.executeStatement("SELECT * FROM allRecordings")
        for rec in recordings:
            experiment = airdb.session.query(
                adb.Experiment).filter_by(string_id=rec['stringId'][1:]).one()
        ...
        if has_corrupt_rec:
            ...
            for corrupt_rec in corrupt_recordings:
                recording = airdb.session.query(
                    adb.Recording).filter_by(id=corrupt_rec['id']).one()
