...
def addExperiment(foldgen,
    ...
    if db.doesTableExist(jobtable) and (db.getNumberOfRecords(jobtable) > 0):
        maxidjob = list(
            db.executeStatement("SELECT max(expId) FROM " + jobtable))
    else:
        maxidjob = []
    print(("Found maximum job id: " + str(maxidjob)))
    if (db.doesTableExist(resulttable)):
        maxidresult = list(
            db.executeStatement("SELECT max(expId) FROM " + resulttable))
    else:
        maxidresult = []
        ...
        if not allowDuplicate:
            if config.countInDb(db, jobtable, "expId") == 0:
                if (not db.doesTableExist(resulttable) or
                    (db.doesTableExist(resulttable)
                     and config.countInDb(db, resulttable, "expId") == 0)):
...
if __name__ == "__main__":
    ...
    parser.add_argument('type',
                        help="character or sentence evaluation",
    ...
    if args.type == "words":
        ...
        config['LMType'] = "grammar"
        config['tokenSequenceModelFile'] = os.path.join(
            hwdir, "flat", "grammar_top99.nav")
    #janus specific
    ...
    config['feataccess'] = os.path.join(hwdir, "flat",
                                        "featAccess.adis.01.tcl")
    ...
    if args.type == 'sentence':
        config['feataccessSen'] = os.path.join(hwdir, "flat",
                                               "featAccess.sqlite.tcl")
        config['dataDirSen'] = os.path.join(hwdir, "data", "sentence")
        config['dictSen'] = os.path.join(hwdir, "vocab",
                                         "dict.en.10k.merg.norepos.sel_v2")
        config['dictChar'] = config['dict']
        config['bigram'] = os.path.join(hwdir, "lm",
                                        "English.vocab.en.10k.merg.sel_v2.lm")
        config['vocabSen'] = os.path.join(hwdir, "vocab",
                                          "vocab.en.10k.merg.sel_v2")
        config['vocabChar'] = config['vocab']
        config['dbSen'] = "db_sentences"
        config[
            'chartrainset'] = "/project/AMR/Handwriting/data/da/l25e/kfold_rh/allset_rh_nrp"
        ...
        experimentStringIds = [
            'v053', 'v054', 'v055', 'v056', 'v057', 'v058', 'v059', 'v060',
            'v061'
        ]
        cvViewName = '_tmpRecordingsAll'
        whereClause = " OR ".join(
            ["stringId = '" + x + "'" for x in experimentStringIds])
    ...
    for conf in confgenerator.getConfigurations():
        ...
        if args.type == 'character':
            ...
            folds = crossValidation.getPerKeyCrossValidationFolds(
                "name", cvViewName)
            addExperiment(folds,
                          conf,
                          dbResults,
