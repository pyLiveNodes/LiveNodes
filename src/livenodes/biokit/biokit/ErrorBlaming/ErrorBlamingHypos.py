...
class TestIntegrationTestSpeech(unittest.TestCase):
    ...
    def test_allTestData(self):
      ...
      def init_pre_processing_chains():
          pre_processing_chain_before_mel.addOperator(self.windowFramer.applyWindow, frameLength, overlap, windowType)
          pre_processing_chain_before_mel.addOperator(self.spectrumCalculator.calculateRealPowerSpectrogram)
          ...
          pre_processing_chain_from_mel.addOperator(
                 ...
                 "vtlnWarpingFactor",
                 False, # janus model
                 True) # this sets the warping factor to be replaced by a value when  pre_processing_chain_from_mel.execute() is executed
          pre_processing_chain_from_mel.addOperator(self.logApplier.applyLog, logK, logA)
          pre_processing_chain_from_mel.addOperator(self.dctTransformer.calculateDct, numDctCoefficients)
          pre_processing_chain_from_mel.addOperator(self.meanSubtraction.subtractMeans, meanSubA)
          pre_processing_chain_from_mel.addOperator(self.adjacentStacking.stackAdjacentFeatureVectors, adjacentStackingDelta)
          pre_processing_chain_from_mel.addOperator(self.ldaTransform.calculateLinearTransform)

      def pre_processing(adcPath, vtlnParam = 1.0):
        log("Info","Preprocessing " + adcPath)
        numChannels=1
        adcOffset=-1
        mcfs1 = self.inputDataFileReader.readFile(adcPath, SF_FORMAT.RAW | SF_FORMAT.PCM_16, samplingRate, numChannels, adcOffset)
        mcfs1 = pre_processing_chain_before_mel.execute(mcfs1)
        mcfs2 = pre_processing_chain_from_mel.execute(mcfs1, vtlnWarpingFactor= vtlnParam)
        return (mcfs1, mcfs2)
      ...
      ldaMatrix = matrix.loadFloatMatrix(dataSpeechPath + "_cd/ldaCSLcd.bmat")
      ldaMatrix = ldaMatrix.transpose(); # saves transposed LDA matrices
      self.ldaTransform.setTransformationMatrix(ldaMatrix);
      numLdaCoefficients = 42
      self.ldaTransform.cutTransformationMatrix(numLdaCoefficients); #cutoff dimensionality for lda
      ...
      modelMapper = ModelMapper.ReadTopology(
          cacheScorer,
          atomMap,
          dataSpeechPath + "_cd/ENdistribTree.2000p",
          dataSpeechPath + "topologyTree",
          dataSpeechPath + "topologies",
      ...
      tokenSequenceModel.readArpaFile(dataSpeechPath + "_lm/English3k.trigram.lm", vocabulary)
      ...
      finalNodeBeam = 120
      finalNodeTopN = 50
      beams = BioKIT.Beams(hypoBeam,hypoTopN,
                           activeNodeBeam,activeNodeTopN,
                           finalNodeBeam,finalNodeTopN)
      tokenSequenceModelWeight = 26.0
      tsmLaTokenSequenceModelWeight = 26.0
      tsmlaCacheSize = 1000
      tokenInsertionPenalty = 0.0
      maxLMLATreeDepth = -1; #important to keep parameter at -1
      searchGraphHandler = SearchGraphHandler(cacheTsm, dictionary,
              vocabulary, modelMapper, cacheScorer,
              beams,
      ...
      searchGraphHandler.getTsmLookAhead().config().setTsmWeight(tsmLaTokenSequenceModelWeight);
      searchGraphHandler.getTsmLookAhead().config().setMaxNodeCache(tsmlaCacheSize);
      ...
      for spkId in open(spkLst):
          ...
          self.meanSubtraction.loadMeans(dataSpeechPath + '_prepro/' + spkId + '.mean', dataSpeechPath + '_prepro/' + spkId + '.smean')

          for info in infos:
              log("Info","Will try to create Snapshot now")
              snapshotName = os.path.join(hypoSnapshotPath,str(info['UTTID'])+".snp")
              refsnapName = os.path.join(refSnapshotPath, str(info['UTTID'])+".snp")

              if os.path.exists(refsnapName) and not os.path.exists(snapshotName):
                searchGraphHandler.createSnapshot(tmpSnapshot, tokenSequenceModelWeight, tokenInsertionPenalty)
                log("Info", "created Snapshot file")

                adcPath = os.path.join(pathPrefix, info['ADC'])
                adcPath = os.path.normpath(adcPath)

                featSeq = pre_processing(adcPath)[1] # 0 = after MEL; 0 = first entry (there should only be one)
                ...
                # get sub, ins and del
                tokenErrorInfo = align.tokenErrorRateInsDelSubCount(reference, cleanedString)
                ...
                log("Info", "Token error information: SPKID: " + str(spkId) + " UTT: " + str(info['UTT']) + " SUB: " + str(sub) + ", INS: " + str(ins) + ", DEL: "+ str(delete) + ", WER: " + str(tokenErrorRate))
                ...
                SnapshotHandler.Shrink(tmpSnapshot, topNHypoSnapshots, snapshotName)
              else:
                print(("no reference snapshot available or hypo snapshot already existing: %s" % refsnapName))

      averageTokenErrorRate = float(totalSub + totalIns + totalDel)/totalWords
      log("Info", "Average token error rate (without VTLN) is: " + str(averageTokenErrorRate))
      log("Info", "Token error information (without VTLN): SUB: " + str(totalSub) + ", INS: "
          + str(totalIns) + ", DEL: "+ str(totalDel)
