...
def storeSubstitution(substitutions, currentReferenceToken, currentHypoToken,
    ...
    if (not currentHypoToken in substitutions[currentReferenceToken]):
        ...
        substitutions[currentReferenceToken][currentHypoToken] = substitutions[
            currentReferenceToken][currentHypoToken] + frequency
    ...
    for side in errorMap[index]:
        for key in errorMap[index][side]:
            ...
            for value in errorMap[index][side][key]:
                print(
                    str(key) + " -> " + str(value) + ": " +
                    str(errorMap[index][side][key][value]))
...
if __name__ == '__main__':
    ...
    if (useSubstitutions == True):
        ...
        while (refsLine != ''):
            ...
            [
                tokenErrorRate, referenceAtFinestStream, hypoAtFinestStream,
                alignedSequences, insDelSubCounts, insDelSubErrors
            ] = align.tokenErrorRateHierarchical(reference, cleanedString)
            for fromStream in range(len(insDelSubErrors[2])):
                for toStream in range(len(insDelSubErrors[2][fromStream])):
                    for dimension in range(
                        ...
                        for tempReference in insDelSubErrors[2][fromStream][
                                toStream][dimension]:
                            for tempHypo in insDelSubErrors[2][fromStream][
                                    toStream][dimension][tempReference]:
                                substitutions[fromStream][toStream][
                                    dimension] = storeSubstitution(
                                        substitutions[fromStream][toStream]
                                        [dimension], tempReference, tempHypo,
                                        insDelSubErrors[2][fromStream]
                                        [toStream][dimension][tempReference]
                                        [tempHypo])
        ...
        for toStream in range(len(substitutions[fromStream])):
            ...
            for dimension in range(len(substitutions[fromStream][toStream])):
                ...
                for key in substitutions[fromStream][toStream][dimension]:
                    for value in substitutions[fromStream][toStream][
                            dimension][key]:
                        amount = substitutions[fromStream][toStream][
                            dimension][key][value]
                        splittedKey = key.split('_')
                        splittedValue = value.split('_')

                        print("bp is: " + splittedKey[2])
                        if (splittedKey[2] == 'wholearm'
                                or splittedValue[2] == 'wholearm'):
                            errorValue = 0.5
                            amount /= 2.0
                        else:
                            errorValue = 1
                        ...
                        for index in range(1, len(splittedKey) - 1):
                            if (splittedKey[index] != splittedValue[index]):
                                ...
                                if splittedKey[index] not in errorMap[index][
                                        splittedKey[3]]:
                                    errorMap[index][splittedKey[3]][
                                        splittedKey[index]] = dict()

                                if splittedValue[index] not in errorMap[index][
                                        splittedKey[3]][splittedKey[index]]:
                                    errorMap[index][splittedKey[3]][
                                        splittedKey[index]][
                                            splittedValue[index]] = errorValue
                                else:
                                    errorMap[index][splittedKey[3]][
                                        splittedKey[index]][splittedValue[
                                            index]] = errorMap[index][
                                                splittedKey[3]][splittedKey[
                                                    index]][splittedValue[
                                                        index]] + errorValue

                                if splittedKey[3] not in bpErrors[index - 1]:
                                    bpErrors[index -
                                             1][splittedKey[3]] = amount
                                else:
                                    bpErrors[index -
                                             1][splittedKey[3]] += amount
