...
def writeMotionPrimitiveCollectionToFile(motionPrimitiveCollection, fileName):
    ...
    for limb in motionPrimitiveCollection:
        ...
        for sequenceType in motionPrimitiveCollection[limb]:
            file.write(
                '<Sequence name="' + sequenceType + '" ' + 'count="' +
                str(len(motionPrimitiveCollection[limb][sequenceType])) + '" ')
            counter = 0
            for primitive in motionPrimitiveCollection[limb][sequenceType]:
                print('Counter: ' + str(counter) + 'Writing primitive: ' +
                      primitive)
                if (counter < 10):
                    file.write('motionPrimitive0' + str(counter) + '="' +
                               primitive + '" ')
                elif (counter < 100) & (counter > 9):
                    file.write('motionPrimitive' + str(counter) + '="' +
                               primitive + '" ')
                else:
                    print(
                        'ERROR: Can not write more than 100 primitives per limb and sequence'
                    )
                counter = counter + 1
            file.write('/>\n')
        file.write('</Limb>\n')
...
def createMotionPrimitiveCollectionFromSegmentations():
    ...
    for subFolder in subfolders:
        ...
        for currentFile in files:
            ...
            xmlMotionLabeling = doc.getElementsByTagName(
                'MotionLabeling').item(0)
            ...
            for xmlLimb in xmlLimbs:
                ...
                for xmlMotionLabel in xmlMotionLabels:
                    ...
                    if (not name
                            in motionPrimitiveCollection[limb][subFolder]):
...
def loadMotionPrimitiveCollection(filename):
    ...
    xmlMotionLabeling = doc.getElementsByTagName(
        'MotionPrimitiveCollection').item(0)
    xmlLimbs = xmlMotionLabeling.getElementsByTagName('Limb')
    for xmlLimb in xmlLimbs:
        limb = xmlLimb.getAttribute('name')
        if (not limb in motionPrimitiveCollection):
        ...
        for xmlMotionSequence in xmlMotionSequences:
            ...
            for primitiveIndex in range(int(numberOfPrimitives)):
                if (primitiveIndex < 10):
                    primitive = xmlMotionSequence.getAttribute(
                        'motionPrimitive0' + str(primitiveIndex))
                elif (primitiveIndex < 100) & (primitiveIndex > 9):
                    primitive = xmlMotionSequence.getAttribute(
                        'motionPrimitive' + str(primitiveIndex))
                else:
                    print(
                        'ERROR: Can not load more than 100 primitives per limb and sequence'
                    )
                motionPrimitiveCollection[limb][sequenceType].append(
                    str.lower(str(primitive)))
...
def retrieveMostLikelySequenceType(streamNamesList, motionPrimitiveCollection,
                                   finestStreamsTokenSequence,
                                   tokenStreamIndices, numberOfFinestStreams):
    ...
    if (len(finestStreamsTokenSequence) > 2):
        ...
        print(
            'ERROR: Method does not guarantee to work correctly for more than 2 finest streams. Correct weighting of finest streams based on body hierarchy is not implemented yet. All finest streams are weighted equally'
        )
        return 'Nothing'
    ...
    for finestStream in range(len(finestStreamsTokenSequence)):
        ...
        for primitiveX in range(len(finestStreamsTokenSequence[finestStream])):
            ...
            for sequenceType in motionPrimitiveCollection[streamName]:
                if finestStreamsTokenSequence[finestStream][
                        primitiveX] in motionPrimitiveCollection[streamName][
                            sequenceType]:
                    if sequenceType in sequenceTypeOccurrences:
                        sequenceTypeOccurrences[
                            sequenceType] += 1.0 / numberOfFinestStreams[
                                streamX]
                    else:
                        sequenceTypeOccurrences[
                            sequenceType] = 1.0 / numberOfFinestStreams[streamX]
    ...
    for type in sequenceTypeOccurrences:
        ...
        if (sequenceTypeOccurrences[type] > mostLikelyCount):
            print('Sequencetype ' + type + ' occurred ' +
                  str(sequenceTypeOccurrences[type]) + ' times')
            mostLikelyCount = sequenceTypeOccurrences[type]
            mostLikelySequenceType = type
    return mostLikelySequenceType


def calculateSequenceAccuracy(tokenSequence, streamNamesList,
                              motionPrimitiveCollection,
    ...
    [
        ...
        firstSubStreamIndex
    ] = align.retrieveHierarchy(splittedTokenSequence)
    ...
    align.retrieveFinestStreamRepresentation(0, firstSubStreamIndex,
    ...
    mostLikelySequenceType = retrieveMostLikelySequenceType(
        streamNamesList, motionPrimitiveCollection, finestStreamsTokenSequence,
        tokenStreamIndices, numberOfFinestStreams)
