...
def lmUpdate():
    ...
    if (createConcurrentTrigrams > 0):
        ...
        for i in lm[3]:
            ...
            lm['p'][i] = math.log(
                (lm[3][i] - disc) /
                lmConcurrent[2][tokens[0] + ',' + tokens[1]]) / 2.30259
...
def lmWrite(filename):
    ...
    if (createConcurrentTrigrams > 0):
        for i in lm[3]:
            ...
            if (tokens[0] + ',' + tokens[1]) not in lm[2]:
                ...
                lm['b'] += 1
            tlist.append(tokens[0].lower() + ' ' + tokens[1].lower() + ' ' +
                         tokens[2].lower() + ' ' + str(lm['p'][i]))
        tlist.sort()
    ...
    if (createConcurrentTrigrams > 0):
        ...
        for t in tlist:
            splittedT = t.split()
            f.write(splittedT[3] + " " + splittedT[0] + " " + splittedT[1] +
                    " " + splittedT[2] + "\n")
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(
        description='Create bigram tokensequence model')
    ...
    parser.add_argument(
        ...
        type=int,
        help=
        'index for the current cross validation fold (-1 = no cross validation)'
    )
    ...
    parser.add_argument(
        '--createConcurrentTrigrams',
        default=0,
        type=int,
        help='create trigrams with predecessor of concurrent primitive')
    ...
    [
        ...
        firstSubStreamIndex
    ] = align.retrieveHierarchy(splittedSequence)
    ...
    for streamIndex in range(numberOfFinestStreams[0]):
        ...
        lmConcurrent[2] = dict()
        lm[1][
            '</s>'] = 0  # Special treatment of final token, which is skipped in lmAcuu
        ...
        for index in range(folds):
            if (index != (cvIndex - 1)):
                ...
                for info in dbInfos:
                    ...
                    referenceSubStream = [[]
                                          for col in range(finestStreamCount)]
                    referenceStreamIndices = [
                        [] for col in range(finestStreamCount)
                    ]

                    align.retrieveFinestStreamRepresentation(
                        0, firstSubStreamIndex, splittedReference,
                        referenceSubStream, referenceStreamIndices,
                        numberOfSubStreams, numberOfFinestStreams)
                ...
                if (createConcurrentTrigrams > 0):
                    ...
                    for cvSequence in cvSequences[index]:
                        ...
                        fileName = str(segFolder) + "/Felix/Ruehrei/" + str(
                            cvSequence) + ".xml"
                        ...
                        tmpLm = dict()
                        [tmpLm[3], tmpLm[2]
                         ] = segmentation.getConcurrentTrigrams(streamIndex)
                        ...
                        for concurrentBigram in tmpLm[2]:
                            if concurrentBigram not in lmConcurrent[2]:
                                lmConcurrent[2][concurrentBigram] = tmpLm[2][
                                    concurrentBigram]
                            else:
                                lmConcurrent[2][concurrentBigram] += tmpLm[2][
                                    concurrentBigram]
        ...
        if (createConcurrentTrigrams > 0):
            lmWrite(addDir + "../../data/langmodConcurrentTrigram_cv" +
                    str(cvIndex) + "_stream" + str(streamIndex))
        else:
            #write tokensequence model to file
            lmWrite(addDir + "../../data/langmodBigram_cv" + str(cvIndex) +
                    "_stream" + str(streamIndex))
