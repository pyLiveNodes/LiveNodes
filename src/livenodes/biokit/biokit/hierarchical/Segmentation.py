...
class Segmentation():
	...
	def createTranscriptionRecursive(self, stream, currentPoint, streamEndPoint):
		result = ''
		...
		while (currentPoint < streamEndPoint):
			...
			if (found == False):
				...
				if (stream in self.topology):
					...
					newEndPoint = streamEndPoint
					if (stream in self.primitives):
						if  (tempPrimitiveIndex < len(self.primitives[stream]) - 1): 
							if ( self.primitives[stream][tempPrimitiveIndex + 1][1] - 1 < streamEndPoint):
								newEndPoint = self.primitives[stream][tempPrimitiveIndex + 1][1] - 1
							
					result = result + '{ ' 
					
					for substreamIndex in range(len(self.topology[stream])):
						substream = self.topology[stream][substreamIndex]
						result = result + str(self.createTranscriptionRecursive(substream, currentPoint, newEndPoint))
						if (substreamIndex < len(self.topology[stream])-1):
							result = result + '| '
					currentPoint = newEndPoint + 1
					result = result + '} '	
				else:
					raise BaseException("Missing token with startPoint " + str(currentPoint) + " in stream " + stream)  
		return result
	...
	def loadFile(self, fileName):
		...
		self.motionSequenceVariant = xmlMotionLabeling.getAttribute('motionSequenceVariant')
	...
	def findConcurrentPrimitive(self, stream, primitive):
		if (len(self.topology) > 1):
			raise BaseException("Can not search for concurrent primitives with more than 2 layers")
		if (len(self.topology[list(self.topology.keys())[0]]) != 2):
			raise BaseException("Can not search for concurrent primitives with != 2 child streams")
		...
		if stream in list(self.topology.keys()):
			...
			if (stream == self.topology[list(self.topology.keys())[0]][0]):
				concurrentStream = self.topology[list(self.topology.keys())[0]][1]
			elif (stream == self.topology[list(self.topology.keys())[0]][1]):
				concurrentStream = self.topology[list(self.topology.keys())[0]][0]
			else:
				raise BaseException("Can not find stream " + stream + " in topology")
			for concurrentPrimitive in self.primitives[concurrentStream]:
				...
				if (concurrentPrimitive[1] <= primitive[1] and concurrentPrimitive[2] >= primitive[1]):
					return concurrentPrimitive
		 	raise BaseException("Could not find concurrent primitive for " + str(primitive))
 	...
 	def findPredecessor(self, streamIndex, primitive):
		...
		for predPrimitive in self.primitives[self.topology[list(self.topology.keys())[0]][streamIndex]]:
			if (predPrimitive[2] == primitive [1]-1):
				return predPrimitive
		raise BaseException("Could not find predecessor primitive for " + str(primitive))
	...
	def getConcurrentTrigrams(self, streamIndex):
		...
		streams.append(self.topology[list(self.topology.keys())[0]][streamIndex])
		
		for stream in streams:
			for primitive in self.primitives[stream]:
				#Find primitive concurrent to start of primitive
				concurrentPrimitive = self.findConcurrentPrimitive(stream, primitive)
				...
				concurrentPredPrimitive = self.findPredecessor(concurrentStreamIndex, concurrentPrimitive)
				
				predPrimitive = self.findPredecessor(streamIndex, primitive)
				
				sequence = concurrentPredPrimitive[0]+','+predPrimitive[0]+','+primitive[0]
		...
		endPrimitive = ['</s>', self.retrieveFinalEndPoint()+1, self.retrieveFinalEndPoint()+2]
		
		if (streamIndex == 1):
			concurrentStreamIndex = 0
		else:
			concurrentStreamIndex = 1
		concurrentPredPrimitive = self.findPredecessor(concurrentStreamIndex, endPrimitive)
		
		predPrimitive = self.findPredecessor(streamIndex, endPrimitive)
		
		sequence = concurrentPredPrimitive[0]+','+predPrimitive[0]+','+endPrimitive[0]
		bigramSequence = concurrentPredPrimitive[0]+','+predPrimitive[0]
...
if __name__ == '__main__':
	...
	file = open('/project/AMR/UpperBody/Recognizers/MotionRecognitionVideoBased/082_FluesPosVarMultiLimb/data/hierarchicalTestData/transcripts_dev_cv1', 'w')
	...
	for subFolder in subfolders:
		...
		for currentFile in files:
			...
			reference = segmentation.createTranscriptionRecursive(mainStream, 0, finalEndPoint)
