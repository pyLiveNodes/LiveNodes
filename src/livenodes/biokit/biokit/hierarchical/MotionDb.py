...
class MotionDb(Database.Database):
    ...
    createTablesStatement = {
        ...
        fileName TEXT)",
        "transcripts":
        "CREATE TABLE transcripts (id INTEGER PRIMARY KEY AUTOINCREMENT, name TEXT, transcript TEXT, \
        FOREIGN KEY(name) REFERENCES sequences(name))",
        "splittedTranscripts":
        "CREATE TABLE splittedTranscripts (id INTEGER PRIMARY KEY AUTOINCREMENT, name TEXT, transcript TEXT, \
        bodyPart TEXT, startPoint INTEGER, endPoint INTEGER, FOREIGN KEY(name) REFERENCES sequences(name))"
    }

    def insertSequence(self, name, sequenceType, sequenceVariant, person,
                       fileName):
    ...
    def insertSplittedTranscript(self, name, transcript, bodyPart, startPoint,
                                 endPoint):
    ...
    def addExperimentFromDirectory(self, directory, personName, mainStream,
                                   topology):
            ...
            if os.path.isdir(os.path.join(os.getcwd(), sequenceType)):
                ...
                for file in filelist:
                    ...
                    if (segmentation.getMotionSequenceVariant() != ""):
                        ...
                        sequenceVariant = sequenceType + re.split(
                            "\d", prefix)[2]
                    ...
                    transcript = segmentation.createTranscriptionRecursive(
                        mainStream, 0, finalEndPoint)

                    if (transcript.lower().strip() != "sil"):

                        self.insertSequence(prefix, sequenceType,
                                            str(sequenceVariant), personName,
                                            prefix)
                        ...
                        for bodyPart in primitives:
                            for primitive in primitives[bodyPart]:
                                self.insertSplittedTranscript(
                                    prefix, str(primitive[0]), str(bodyPart),
                                    primitive[1], primitive[2])

                        count += 1
                    else:
                        self.log("info",
                                 "Skipping file " + file + " containing sil")
                os.chdir("..")
        self.log("info",
                 "inserted " + str(count) + " recordings into database.")
    ...
    def retrieveBalancedCrossValidationSetsLeaveOneOut(self, key):
        ...
        self.log(
            "info",
            "Retrieving crossvalidation set with Leave one out strategy for key "
            + str(key))
        ...
        for res in variances:
            cvSequences.append([])
            sqlcmd = "SELECT name FROM sequences WHERE " + key + " = '" + res[
                key] + "'"
            result = self.executeStatement(sqlcmd)
            for res in result:
    ...
    def retrieveBalancedCrossValidationSets(self, key, folds):
        self.log(
            "info", "Retrieving balanced crossvalidation set with " +
            str(folds) + " folds for key " + str(key))
        sqlcmd = "SELECT DISTINCT " + key + " FROM sequences"
        self.logsql(sqlcmd)
        variances = self.executeStatement(sqlcmd)
        ...
        for res in variances:
            ...
            for i in range(folds):
                for j in range(int(round(i * foldSize)),
                               int(round((i + 1) * foldSize))):
                    cvSequences[i].append(iterator.next()['name'])
        return cvSequences

    def retrieveBalancedCrossValidationSetsForSelection(
            self, key, folds, selection):
        self.log(
            "info", "Retrieving balanced crossvalidation set with " +
            str(folds) + " folds for key " + str(key))
        ...
        for res in variances:
            sqlcmd = "SELECT name FROM sequences WHERE " + key + " = '" + res[
                key] + "' AND name IN ( " + sequenceList + " ) ORDER BY name"
            self.logsql(sqlcmd)
            result = self.executeStatement(sqlcmd)
            self.logres(result)
            foldSize = (len(result) * 1.0) / folds
    ...
    def writeJanusPreDatabase(self,
                              key,
                              dataSet,
                              cvIndex,
                              folds,
        ...
        if ((dataSet == 'init') or (dataSet == 'train')):
            for part in range(partitions):
                ...
                fileName = os.path.join(
                    prefix, "pre-dbase_" + dataSet + "_part1to" +
                    str(part + 1) + "_cv" + str(cvIndex + 1))
                ...
                limitedCvSequences = self.retrieveBalancedCrossValidationSetsForSelection(
                    key, partitions, accumulatedCvSequences)
                for j in range(part + 1):
                    self.log(
                        "info",
                        "Writing " + dataSet + "file for cvIndex " + str(i))
                    self.writeFoldToJanusPreDatabase(limitedCvSequences, j,
                                                     fileName, bodyPart,
                                                     resample,
                                                     trainSequenceTypes,
                                                     splitted)
            ...
            fileName = os.path.join(
                prefix, "pre-dbase_" + dataSet + "_cv" + str(cvIndex + 1))
            if (os.path.exists(fileName)):
                os.remove(fileName)
            self.log("info",
                     "Writing " + dataSet + "file for cvIndex " + str(cvIndex))
            self.writeFoldToJanusPreDatabase(cvSequences, cvIndex, fileName,
                                             bodyPart, resample,
                                             trainSequenceTypes, splitted)

    def writeFoldToJanusPreDatabase(self, cvSequences, cvIndex, fileName,
                                    bodyPart, resample, trainSequenceTypes,
                                    splitted):
        self.log("info",
                 "Writing fold " + str(cvIndex) + " to Janus PreDatabase")
        if (splitted == True):
        ...
        for transcript in transcripts:
            if ((transcript['transcript'].lower().strip() != 'sil')
                    and (transcript['sequenceType'] in trainSequenceTypes)):
