...
class Draw_recognition(View):
    ...
    example_init = {"name": "Recognition", "xAxisLength": [50, 50, 50, 5000]}
    ...
    def _init_draw(self, subfig):
        ...
        def update(recognition, annotation, colors):
            ...
            self.names[0], self.verts[0] = convert_list_pos(
                states[-self.xAxisLength[0]:], self.xAxisLength[0], (0, 0.7))
            self.names[1], self.verts[1] = convert_list_pos(
                atoms[-self.xAxisLength[1]:], self.xAxisLength[1], (0, 0.7))
            self.names[2], self.verts[2] = convert_list_pos(
                tokens[-self.xAxisLength[2]:], self.xAxisLength[2], (0, 0.7))

            if annotation is not None:
                self.names[3], self.verts[3] = convert_list_pos(
                    annotation[-self.xAxisLength[3]:], self.xAxisLength[3],
                    (0, 0.7))
