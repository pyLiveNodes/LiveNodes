...
def max_power_spectrum(wts, samplingfrequency):
    ...
    return np.max(scipy.signal.welch(wts, int(samplingfrequency), nperseg=wts.shape[-1], axis=-1)[1], axis=-1)
...
def spectral_skewness(wts, samplingfrequency):
    ...
    return divideWithZero(np.sum(skew, axis=-1), spectral_spread(wts, samplingfrequency) ** 3)
...
def spectral_entropy(wts, samplingfrequency):
    ...
    power = fmag ** 2
    prob = np.divide(power, matchLastDimByRepeat(np.sum(power, axis=-1), power))
...
def fft_mean_coeff(wts, samplingfrequency, nfreq: int = 256):
    ...
    return np.mean(scipy.signal.spectrogram(wts, samplingfrequency, nperseg=nfreq * 2 - 2, axis=-1)[2], axis=-1)
