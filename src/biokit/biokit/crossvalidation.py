...
class CrossValidation:
    ...
    def cutListToMinimum(self, list, min_val=None):
        ...
        """
        returnlist = [[item for item in list if item[self.classColumn] == x] for x in self.classes]
        minimum = min([len(returnlist[x]) for x in self.classCount])
        if min_val is not None and min_val < minimum:
            minimum = min_val
        print(("CrossValidation: number of samples per class is %s" %
              minimum))
        returnlist = [random.sample(returnlist[x], minimum) for x in self.classCount]
        return returnlist
    
    def createKFoldCrossvalidation(self, inputList, classColumn, K = None,
                                   maxsamples = None):
        """
        ...
        if (classColumn >= 0):
            ...
            self.classes = list(set([element[classColumn] for element in inputList]))
            ...
            if (K == None):
                K = len(final[0])
            for k in range(K):
                training = [[value for i, value in enumerate(final[subClass]) if i % K != k] for subClass in self.classCount]
                training = [item for sublist in training for item in sublist]
                validation = [[value for i, value in enumerate(final[subClass]) if i % K == k] for subClass in self.classCount]
            ...
            for k in range(K):
                training = [value for i, value in enumerate(inputList) if i % K != k]
                validation = [value for i, value in enumerate(inputList) if i % K == k]
                yield({'train' : training,'test' : validation})
            
    def createPerKeyCrossvalidation(self, inputList, keyColumn, classColumn = -1):
        """
        ...
        listPerKey = [[item for item in inputList if item[keyColumn] == x] for x in self.keys]
        #do we have a class column?
        if (classColumn >= 0):
            self.classColumn = classColumn
            self.classes = list(set([element[classColumn] for element in inputList]))
            ...
            absoluteMinimum = min([len(self.cutListToMinimum(x)[0]) for x in listPerKey])

            final = [self.cutListToMinimum(listPerKey[x], absoluteMinimum) for x in keyCount]
            for i in keyCount:
                training = final[:i] + final[i+1:]
                training = [item for sublist in training for item in sublist]
                training = [item for sublist in training for item in sublist]
                validation =  [item for sublist in final[i] for item in sublist]
                yield({'train' : training,'test' : validation, 'keyValue':validation[0][keyColumn], 'keyCount':len(keyCount)})
        else:
