...
gaussianContainerSet.readDescFile("/home/erhardt/c++workspace/decoder-data/Weights/mas.cbsDesc")
gaussianContainerSet.loadDataFile("/home/erhardt/c++workspace/decoder-data/Weights/6.cbs")

log("Info","Gaussian container set is: " + str(gaussianContainerSet))
gaussMixturesSet = GaussMixturesSet(gaussianContainerSet)
gaussMixturesSet.readDescFile("/home/erhardt/c++workspace/decoder-data/Weights/mas.dssDesc")
gaussMixturesSet.loadDataFile("/home/erhardt/c++workspace/decoder-data/Weights/6.dss")
...
while i < len(infoList):
	...
	if(os.path.exists(referencesPath + str(info['UTTID'])+ ".snp") and os.path.exists(snapshotsPath + str(info['UTTID']) + ".snp")):
		log("Info","Utterance "+ str(info['UTTID']) + " has complete dataset.")
	...
	i = i+1
resultFile = open(resultPath+"SEAME-confusionTable-"+str(datetime.datetime.today().isoformat())+".csv",'w')
