...
if __name__ == "__main__":
    ...
    if not config.basemodel:
        log("Info", "No initial model given, checking for existing initial models")
        try:
    ...
    with open(trainlog, "w") as logfh:
        log("Info", "Training with Janus for %s iterations" % 
            (config.iterations, ))
    ...
    for iter, modelparams in list(existingModels.items()):
        if not modelparams:
            ...
            modelparam.read_from_files("codebookSet",
                                       "codebookWeights.%s" % iter, 
                                       "distribSet",
                                       "distribWeights.%s" % iter, 
                                       "distribTree", "topologies",
                                       "topologyTree", "transitionModels",
                                       "phonesSet")
            ...
            log("Info", "Models %s for iteration %s already existed." % (modelparams, iter))
    ...
    if config.testset:
        if config.biokitconfig and not config.ibisconfig:
            ...
            for iter in range(config.iterations+1):
                ...
                log("Info", "Token Error Rate: " + str(ter))
                add_results_to_db(airdb.session, ter, iter, config, resultslist)
        else:
