...
if __name__ == '__main__':
    ...
    worker_term_lock.acquire()
    # worker = Process(target=partial(start, pipeline, worker_term_lock))
    worker = Thread(target=partial(start, pipeline, worker_term_lock))
    worker.start()
    ...
    fig.canvas.mpl_connect("close_event", partial(stop, worker_term_lock, worker))
    # plt.tight_layout()
    # plt.ioff()
    plt.show()
